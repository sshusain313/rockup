...
export function TankTopGallery({
  activeCategory,
  setActiveCategory,
  uploadedDesign,
  products = []
  ...
  const handleMockupClick = (mockup: MockupItem) => {
    ...
    if (uploadedDesign) {
      ...
      router.push(`/editor/tanktop?productId=${mockup.id}`);
    } else {
      router.push(`/editor/tanktop?productId=${mockup.id}`);
    }
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 w-full">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold">Tank Top Gallery</h2>
          ...
          {filteredMockups.map((mockup) => (
            ...
            >
              <div className="relative aspect-square overflow-hidden">
                {mockup.image ? (
                  <Link
                    href={`/editor/tanktop?productId=${mockup.id}${uploadedDesign ? `&designImage=${encodeURIComponent(uploadedDesign)}` : ''}`}
                    onClick={() => {
                      localStorage.setItem('selectedProductId', mockup.id);
                  ...
                  >
                    <Image
                      src={mockup.image}
                      alt={mockup.title || 'Tank Top product'}
                      fill
              ...
              <div className="p-3">
                <div className="text-sm text-gray-700 truncate font-medium">{mockup.title || 'Tank Top product'}</div>
                {mockup.categories && mockup.categories.some(cat =>
                  ['White', 'Black', 'Red', 'Blue', 'Green', 'Yellow', 'Navy Blue', 'Gray'].includes(cat)
                ) && (

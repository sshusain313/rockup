...
export const PlaceholderDesignOverlay: React.FC<PlaceholderDesignOverlayProps> = ({
  ...
  const calculateDesignStyle = () => {
    ...
    // Apply additional adjustments based on tank top categories
    let adjustedTop = top;
    let adjustedLeft = left;
    let rotation = 0;
    let perspective = '';
    
    // Adjust vertical position based on tank top type
    if (categories.includes("Oversized")) {
      // Move design slightly higher on oversized tank tops
      adjustedTop = top - (mockupHeight * 0.01); // Slightly more adjustment for tank tops
    } else if (categories.includes("Closeup")) {
      // Center design more precisely on closeups
      adjustedTop = top - (mockupHeight * 0.005);
    } else if (categories.includes("Mannequin")) {
      // Adjust for mannequin perspective
      adjustedTop = top - (mockupHeight * 0.01);
      perspective = 'perspective(500px) rotateX(7deg)'; // Slightly more rotation for tank tops
    }
    
    // Adjust for side view
    if (categories.includes("Side View")) {
      rotation = 18; // Slightly more rotation for tank tops
      adjustedLeft = left - (mockupWidth * 0.01);

          ...
          {filteredMockups.map((mockup) => (
            ...
            >
              <div className="relative aspect-square overflow-hidden">
                {mockup.image ? (
                  <Link
                    href={`/editor?productId=${mockup.id}${uploadedDesign ? `&designImage=${encodeURIComponent(uploadedDesign)}` : ''}`}
                    onClick={() => {
                      ...
                      // Ensure consistent placeholder data
                      ...
                      // Store the placeholder in multiple formats for consistency
                      localStorage.setItem('productPlaceholder', JSON.stringify(placeholder));
                      
                      // Also store as percentage-based coordinates for better scaling
                      ...
                      // Store categories for blend mode consistency
                      localStorage.setItem('productCategories', JSON.stringify(mockup.categories || []));
                      
                      // Store custom shape points if available
                      if (mockup.customShapePoints && mockup.customShapePoints.length > 0) {
                        localStorage.setItem('productCustomShapePoints', JSON.stringify(mockup.customShapePoints));
                      }
                    }}
                  >
                    <Image
                      src={mockup.image}
                      alt={mockup.title || 'T-shirt product'}
                      fill
              ...
              <div className="p-3">
                <div className="text-sm text-gray-700 truncate font-medium">{mockup.title || 'T-shirt product'}</div>
                {mockup.categories && mockup.categories.some(cat =>
                  ['White', 'Black', 'Red', 'Blue', 'Green', 'Yellow', 'Navy Blue', 'Gray'].includes(cat)
                ) && (
